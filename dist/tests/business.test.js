...
it('has a seeded store and product catalog that is queryable', () => {
    const store = getDatabase().prepare('SELECT COUNT(*) as count FROM stores').get();
    const products = getDatabase().prepare('SELECT COUNT(*) as count FROM products').get();
