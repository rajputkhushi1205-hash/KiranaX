...
export function routeCustomers() {
  ...
  router.post('/', (req, res) => {
    ...
    try {
      ...
      return res.status(409).json({ success: false, error: { code: 'CUSTOMER_EXISTS', message: (error as Error).message } });
    }
  });
  ...
  router.get('/:id', (req, res) => {
    try {
      ...
      return res.status(404).json({ success: false, error: { code: 'CUSTOMER_NOT_FOUND', message: (error as Error).message } });
    }
  });
