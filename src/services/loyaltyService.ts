...
export function evaluateLoyalty(customerId: string, subtotal: number) {
...
export function getLoyaltyRules(storeId: string) {
  const db = getDatabase();
  return db.prepare('SELECT * FROM loyalty_rules WHERE store_id = ? AND active = 1').all(storeId) as any[];
}
